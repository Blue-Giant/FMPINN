...
def solve_Multiscale_PDE(R):
    ...
    region_lb = 0.0
    region_rt = 1.0
    if R['equa_name'] == 'multi_scale3D_6':
        u_true, f, A_eps, u_left, u_right, u_front, u_behind, u_bottom, u_top = MS_LaplaceEqs.get_infos2MS_Laplace3D_E6(
            eps=R['epsilon'])
    elif R['equa_name'] == 'multi_scale3D_7':
        u_true, f, A_eps, u_left, u_right, u_front, u_behind, u_bottom, u_top = MS_LaplaceEqs.get_infos2MS_Laplace3D_E7(
            s=0.25, eps=R['epsilon'])
    else:
    ...
    for i_epoch in range(R['max_epoch'] + 1):
        ...
        if R['equa_name'] == 'multi_scale3D_4':
            ...
            force_side = MS_LaplaceEqs.get_force2multi_scale3D_E5(x=XX, y=YY, z=ZZ)
            UNN2train, loss_it, loss_dAU = model.loss_in2pLaplace(
                XYZ=xyz_it_batch, fside=force_side, if_lambda2fside=False, aeps=A_eps,
                if_lambda2aeps=True, loss_type=R['loss_type'], scale2lncosh=R['scale2lncosh'])
        elif R['equa_name'] == 'multi_scale3D_6':
            force_side = MS_LaplaceEqs.get_force_side2MS_Laplace2E6_3D(x=XX, y=YY, z=ZZ, eps=R['epsilon'])
            UNN2train, loss_it, loss_dAU = model.loss_in2pLaplace(
                XYZ=xyz_it_batch, fside=force_side, if_lambda2fside=False, aeps=A_eps,
                if_lambda2aeps=True, loss_type=R['loss_type'], scale2lncosh=R['scale2lncosh'])
        elif R['equa_name'] == 'multi_scale3D_7':
            force_side = MS_LaplaceEqs.get_force_side2MS_Laplace2E7_3D(x=XX, y=YY, z=ZZ, s=0.25, eps=R['epsilon'])
            UNN2train, loss_it, loss_dAU = model.loss_in2pLaplace(
                XYZ=xyz_it_batch, fside=force_side, if_lambda2fside=False, aeps=A_eps,
                if_lambda2aeps=True, loss_type=R['loss_type'], scale2lncosh=R['scale2lncosh'])
        else:
