...
class MscaleDNN(tn.Module):
    ...
    def load_test_data2mat(self, eps=0.1, slice_index2z=20, with_gpu=True):
        ...
        U_downsample = u_ref[:, :, slice_index2z]
        ...
        z = coords[slice_index2z] * torch.ones_like(x)
        torch_XYZ = torch.cat([x, y, z], dim=-1)
        if with_gpu:
            XYZ=torch_XYZ.to(self.opt2device)
