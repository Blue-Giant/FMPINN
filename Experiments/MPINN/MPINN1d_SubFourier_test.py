...
class MscaleDNN(tn.Module):
    def __init__(self, input_dim=4, out_dim=1, hidden_layer=None, Model_name='DNN', name2actIn='relu',
        ...
        if 'FOURIER_DNN' == str.upper(Model_name):
    ...
    def loss_in2pLaplace(self, X=None, fside=None, if_lambda2fside=True, aeps=None, if_lambda2aeps=True,
                         loss_type='ritz_loss', scale2lncosh=0.5, eps=0.001):
        """
        ...
        tensor_eps = torch.tensor([eps], dtype=self.float_type,
                                  device=self.opt2device, requires_grad=False)  # 3 行 1 列
        temp1 = 2.0*X*(2.0+torch.sin(2.0*torch.pi*X/tensor_eps)) - \
                (1+X*X)*torch.sin(2.0*torch.pi*X/tensor_eps)*2.0*torch.pi/tensor_eps
        temp2 = (2.0+torch.sin(2.0*torch.pi*X/tensor_eps))*(2.0+torch.sin(2.0*torch.pi*X/tensor_eps))
        dAdx = temp1/temp2
...
def solve_Multiscale_PDE(R):
    ...
    for i_epoch in range(R['max_epoch'] + 1):
        ...
        UNN2train, loss_it = model.loss_in2pLaplace(
            ...
            scale2lncosh=R['scale2lncosh'], eps=R['epsilon'])
...
if __name__ == "__main__":
    ...
    R['batch_size2boundary'] = 200                   # 边界训练数据大小
