...
class MscaleDNN(tn.Module):
    ...
    def loss_in2pLaplace(self, XY=None, fside=None, if_lambda2fside=True, aeps=None, if_lambda2aeps=True,
        ...
        ddUNN2dy = torch.reshape(grad_dUNN2dy[:, 1], shape=[-1, 1])
        ...
        gradA = torch.autograd.grad(aeps_side, XY, grad_outputs=torch.ones_like(X), create_graph=True,
                                    retain_graph=True)[0]
...
def solve_Multiscale_PDE(R):
    ...
    for i_epoch in range(R['max_epoch'] + 1):
        xy_it_batch = dataUtilizer2torch.rand_in_2D(
            ...
            region_bottom=region_lb, region_top=region_rt, to_torch=True, to_float=True, to_cuda=R['use_gpu'],
            gpu_no=R['gpuNo'])
