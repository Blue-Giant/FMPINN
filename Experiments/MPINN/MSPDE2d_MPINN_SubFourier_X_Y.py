...
class MscaleDNN(tn.Module):
    ...
    def loss_in2pLaplace(self, X=None, Y=None, fside=None, if_lambda2fside=True, aeps=None, if_lambda2aeps=True,
        ...
        grad_dUNN2dx = torch.autograd.grad(dUNN2dx, X, grad_outputs=torch.ones_like(X), create_graph=True,
                                           retain_graph=True)
        grad_dUNN2dy = torch.autograd.grad(dUNN2dy, Y, grad_outputs=torch.ones_like(X), create_graph=True,
                                           retain_graph=True)

        ddUNN2dx = torch.reshape(grad_dUNN2dx[0], shape=[-1, 1])
        ddUNN2dy = torch.reshape(grad_dUNN2dy[0], shape=[-1, 1])
        ...
        AddUNN = torch.multiply(aeps_side, torch.add(ddUNN2dx, ddUNN2dy))
...
def solve_Multiscale_PDE(R):
    ...
    for i_epoch in range(R['max_epoch'] + 1):
        x_it_batch = dataUtilizer2torch.rand_it_lhs(batchsize_it, 1, region_a=region_lb, region_b=region_rt,
                                                    ...
                                                    use_grad2x=True)
        y_it_batch = dataUtilizer2torch.rand_it_lhs(batchsize_it, 1, region_a=region_lb, region_b=region_rt,
                                                    to_float=True, to_cuda=R['use_gpu'], gpu_no=R['gpuNo'],
                                                    use_grad2x=True)
        # 边界采用随机采样点方法，比较好
...
if __name__ == "__main__":
    ...
    # R['equa_name'] = 'multi_scale2D_9'      # p=2 区域为 [-1,1]X[-1,1] 论文中的两尺度例子，eps=0.5
    R['equa_name'] = 'multi_scale2D_10'        # p=2 区域为 [-1,1]X[-1,1] 论文中的两尺度例子，eps=0.05
