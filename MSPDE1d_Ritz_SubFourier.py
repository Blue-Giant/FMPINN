...
class MscaleDNN(tn.Module):
    ...
    def loss_in2pLaplace(self, X=None, fside=None, if_lambda2fside=True, aeps=None, if_lambda2aeps=True,
        ...
        grad2UNN = torch.autograd.grad(UNN, X, grad_outputs=torch.ones_like(X),
                                       create_graph=True, retain_graph=True)
        dUNN = torch.reshape(grad2UNN[0], shape=[-1, 1])
        ...
        dUNN_2Norm = torch.reshape(torch.mul(dUNN, dUNN), shape=[-1, 1])  # 按行求和
...
if __name__ == "__main__":
    ...
    if R['model2NN'] == 'Fourier_DNN':
        ...
        if R['epsilon'] == 0.005:
            # R['hidden_layers'] = (20, 40, 30, 30, 15)
            # R['freq'] = np.concatenate(([1], np.arange(start=2, stop=101, step=4)), axis=0)
            R['hidden_layers'] = (30, 40, 30, 30, 30)
            # R['freq'] = np.concatenate(([1], np.arange(start=2, stop=100, step=5), [100]), axis=0)
            R['freq'] = np.concatenate(([1, 2, 3, 4, 5], np.arange(start=10, stop=100, step=5), [100]), axis=0)
        if R['epsilon'] == 0.001:
            # R['hidden_layers'] = (30, 50, 30, 30, 15)
            R['hidden_layers'] = (30, 40, 30, 30, 30)
    ...
    if R['model2NN'] == 'Fourier_SubDNN' and R['name2act_hidden'] == 'tanh':
        # R['sfourier'] = 0.5
        R['sfourier'] = 1.0
    elif R['model2NN'] == 'Fourier_SubDNN' and R['name2act_hidden'] == 's2relu':
        R['sfourier'] = 0.5
        # R['sfourier'] = 1.0
    elif R['model2NN'] == 'Fourier_SubDNN' and R['name2act_hidden'] == 'sinAddcos':
        # R['sfourier'] = 0.5
        R['sfourier'] = 1.0
    elif R['model2NN'] == 'Fourier_SubDNN' and R['name2act_hidden'] == 'sin':
        # R['sfourier'] = 0.5
        R['sfourier'] = 1.0
    else:
        R['sfourier'] = 1.0
