...
class MscaleDNN(tn.Module):
    ...
    def loss_in2pLaplace(self, XYZ=None, fside=None, if_lambda2fside=True, aeps=None, if_lambda2aeps=True,
        ...
        grad2UNNX = torch.autograd.grad(dUNN2dx, XYZ, grad_outputs=torch.ones_like(X), create_graph=True,
                                        retain_graph=True)[0]
        grad2UNNY = torch.autograd.grad(dUNN2dy, XYZ, grad_outputs=torch.ones_like(X), create_graph=True,
                                        retain_graph=True)[0]

        grad2UNNZ = torch.autograd.grad(dUNN2dz, XYZ, grad_outputs=torch.ones_like(X), create_graph=True,
                                        retain_graph=True)[0]

        ddUNN2dx = torch.reshape(grad2UNNX[:, 0], shape=[-1, 1])
        ddUNN2dy = torch.reshape(grad2UNNY[:, 1], shape=[-1, 1])
        ddUNN2dz = torch.reshape(grad2UNNZ[:, 2], shape=[-1, 1])
        ...
        gradA = torch.autograd.grad(aeps_side, XYZ, grad_outputs=torch.ones_like(X), create_graph=True,
                                    retain_graph=True)[0]

        dA2dx = torch.reshape(gradA[:, 0], shape=[-1, 1])
        dA2dy = torch.reshape(gradA[:, 1], shape=[-1, 1])
        dA2dz = torch.reshape(gradA[:, 2], shape=[-1, 1])
...
def solve_Multiscale_PDE(R):
    ...
    for i_epoch in range(R['max_epoch'] + 1):
        xyz_it_batch = dataUtilizer2torch.rand_it_lhs(
            batchsize_it, R['input_dim'], region_a=region_lb, region_b=region_rt, to_float=True, to_cuda=R['use_gpu'],
