...
class MscaleDNN(tn.Module):
    ...
    def loss_in2pLaplace(self, X=None, Y=None, Z=None, fside=None, if_lambda2fside=True, aeps=None, if_lambda2aeps=True,
        ...
        grad2UNNX = torch.autograd.grad(dUNN2dx, X, grad_outputs=torch.ones_like(X), create_graph=True,
                                        retain_graph=True)
        grad2UNNY = torch.autograd.grad(dUNN2dy, Y, grad_outputs=torch.ones_like(X), create_graph=True,
                                        retain_graph=True)

        grad2UNNZ = torch.autograd.grad(dUNN2dz, Z, grad_outputs=torch.ones_like(X), create_graph=True,
                                        retain_graph=True)

        ddUNN2dx = torch.reshape(grad2UNNX[0], shape=[-1, 1])
        ddUNN2dy = torch.reshape(grad2UNNY[0], shape=[-1, 1])
        ddUNN2dz = torch.reshape(grad2UNNZ[0], shape=[-1, 1])
        ...
        gradX2A = torch.autograd.grad(aeps_side, X, grad_outputs=torch.ones_like(X), create_graph=True,
                                      retain_graph=True)
        gradY2A = torch.autograd.grad(aeps_side, Y, grad_outputs=torch.ones_like(X), create_graph=True,
                                      retain_graph=True)
        gradZ2A = torch.autograd.grad(aeps_side, Z, grad_outputs=torch.ones_like(X), create_graph=True,
                                      retain_graph=True)

        dA2dx = torch.reshape(gradX2A[0], shape=[-1, 1])
        dA2dy = torch.reshape(gradY2A[0], shape=[-1, 1])
        dA2dz = torch.reshape(gradZ2A[0], shape=[-1, 1])

        dAdx_dUNNdx = torch.multiply(dA2dx, dUNN2dx)
        dAdy_dUNNdy = torch.multiply(dA2dy, dUNN2dy)
        dAdz_dUNNdz = torch.multiply(dA2dz, dUNN2dz)

        ddUNN = ddUNN2dx+ddUNN2dy+ddUNN2dz
        AddUNN = torch.multiply(aeps_side, ddUNN)

        loss2func_temp = dAdx_dUNNdx + dAdy_dUNNdy + dAdz_dUNNdz + AddUNN + force_side
...
def solve_Multiscale_PDE(R):
    ...
    for i_epoch in range(R['max_epoch'] + 1):
        x_it_batch = dataUtilizer2torch.rand_it_lhs(
            batchsize_it, 1, region_a=region_lb, region_b=region_rt, to_float=True, to_cuda=R['use_gpu'],
            gpu_no=R['gpuNo'], use_grad2x=True)
        y_it_batch = dataUtilizer2torch.rand_it_lhs(
            batchsize_it, 1, region_a=region_lb, region_b=region_rt, to_float=True, to_cuda=R['use_gpu'],
            gpu_no=R['gpuNo'], use_grad2x=True)
        z_it_batch = dataUtilizer2torch.rand_it_lhs(
            batchsize_it, 1, region_a=region_lb, region_b=region_rt, to_float=True, to_cuda=R['use_gpu'],
            gpu_no=R['gpuNo'], use_grad2x=True)
